...
int main(int argc, char *argv[]) {
  ...
  struct workerMax dataMax;
  struct workerMin dataMin; /* partial data for min and max values */
...
#pragma omp parallel for reduction (+:total) reduction (customMax:dataMax) reduction (customMin:dataMin) private(j)
  for (i = 0; i < size; i++)
    for (j = 0; j < size; j++){
      ...
      if(matrix[i][j]<dataMin.min){
        dataMin.min = matrix[i][j];
        dataMin.minPos[0] = i;
        dataMin.minPos[1] = j;
      }
